...
export const AboutSection: React.FC<AboutSectionProps> = ({ 
  ...
  const handleUpdatePillar = (id: string, field: 'title' | 'description', value: string) => {
    ...
    const updated = approachPillars.map((p) => (p.id === id ? { ...p, [field]: value } : p));
    onUpdateProfile({ approachPillars: updated });
  };

  return (
    <section id="about" className="py-20 md:py-24 bg-[#FAF6EF] border-t border-[#1B4332]/20">
            ...
            <div className="relative group w-full sm:w-56 lg:w-full">
              <div className="w-full aspect-[4/5] rounded-sm overflow-hidden border border-[#1B4332]/25 bg-[#F4EFE6] shadow-sm relative">
                ...
                <button
                  ...
                  className="absolute bottom-2 right-2 px-2.5 py-1.5 rounded-sm bg-[#FAF6EF]/95 hover:bg-[#FAF6EF] text-[#1B4332] text-[11px] font-bold tracking-wider uppercase border border-[#1B4332]/30 shadow-sm flex items-center gap-1.5 transition-all cursor-pointer opacity-90 hover:opacity-100"
                  title="Upload headshot photo"
                >
          ...
          <div className="lg:col-span-8 space-y-6">
            ...
            <div>
              <h2 className="text-2xl sm:text-4xl md:text-5xl font-serif font-normal text-[#18221B] tracking-tight leading-[1.15]">
                <EditableText
                  value={profile.aboutHeadline || 'Taking raw data to actionable insight, and real change.'}
        ...
        <div className="pt-6 border-t border-[#1B4332]/15">
          <div className="grid grid-cols-1 sm:grid-cols-3 gap-4">
            {approachPillars.map((pillar, idx) => {
              const Icon = idx === 0 ? TrendingUp : idx === 1 ? Layers : CheckCircle2;
              return (
                <div key={pillar.id} className="p-4 rounded-sm bg-[#F4EFE6] border border-[#1B4332]/20 space-y-1.5 shadow-2xs">
                  ...
                  <h4 className="text-sm font-serif font-bold text-[#18221B]">
                    <EditableText
                      value={pillar.title}
                      onSave={(val) => handleUpdatePillar(pillar.id, 'title', val)}
                      isEditMode={isEditMode}
                      as="span"
                    />
                  </h4>
                  <p className="text-[11px] text-[#4F6355] leading-relaxed">
                    <EditableText
                      ...
                      onSave={(val) => handleUpdatePillar(pillar.id, 'description', val)}
                      isEditMode={isEditMode}
                      multiline={true}
                      as="span"
                    />
